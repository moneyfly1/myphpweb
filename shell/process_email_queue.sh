...
if [[ "$PROJECT_PATH" == *"/www/wwwroot/"* ]] || [ -d "/www/wwwroot" ] || [[ "$(pwd)" == *"/www/wwwroot/"* ]] || [ -f "/etc/redhat-release" ] || [ -f "/etc/debian_version" ]; then
    ...
    if [[ "$PROJECT_PATH" != *"/www/wwwroot/"* ]]; then
        ...
        if [ -z "$BT_SITE" ]; then
            for site_dir in /www/wwwroot/*; do
                if [ -d "$site_dir" ] && [ -f "$site_dir/shell/process_email_queue.php" ]; then
                    BT_SITE="$site_dir"
                    break
                fi
            done
    ...
    LOG_FILE="/var/log/email_queue.log"
    ERROR_LOG="/var/log/email_queue_error.log"
    echo "检测到生产环境（宝塔面板），使用系统日志目录"
else
    # 本地开发环境
    # 自动检测PHP路径
    if command -v php >/dev/null 2>&1; then
    ...
    LOG_FILE="$PROJECT_PATH/logs/email_queue.log"
    ERROR_LOG="$PROJECT_PATH/logs/email_queue_error.log"
...
# 检查PHP文件是否存在
if [ ! -f "$PROJECT_PATH/shell/process_email_queue.php" ]; then
    log_error "邮件队列处理文件不存在: $PROJECT_PATH/shell/process_email_queue.php"
    exit 1
fi
