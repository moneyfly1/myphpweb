...
# 执行日志轮转
log_message "执行日志轮转..."
if php shell/log_manager.php rotate >> "$LOG_FILE" 2>&1; then
    log_message "日志轮转完成"
else
...
# 清理过期日志
log_message "清理过期日志..."
if php shell/log_manager.php clean >> "$LOG_FILE" 2>&1; then
...
# 监控日志大小
log_message "监控日志大小..."
if php shell/log_manager.php monitor >> "$LOG_FILE" 2>&1; then
    log_message "日志监控完成"
else
