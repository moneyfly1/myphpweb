...
class OrderController extends AdminBaseController
{
    ...
    public function payedit()
    {
        if (IS_POST) {
            ...
            if ($result) {
                // 如果是支付宝配置，同步到.env文件
                if (isset($temp['pay_type']) && $temp['pay_type'] === 'zfb' || (isset($data['app_id']) && isset($data['alipay_public_key']))) {
                    $envData = [];
                    if (isset($data['app_id']))
                        $envData['ALIPAY_APP_ID'] = $data['app_id'];
                    if (isset($data['merchant_private_key']))
                        $envData['ALIPAY_PRIVATE_KEY'] = $data['merchant_private_key'];
                    if (isset($data['alipay_public_key']))
                        $envData['ALIPAY_PUBLIC_KEY'] = $data['alipay_public_key'];
                    if (isset($data['return_url']))
                        $envData['ALIPAY_RETURN_URL'] = $data['return_url'];
                    if (isset($data['notify_url']))
                        $envData['ALIPAY_NOTIFY_URL'] = $data['notify_url'];

                    if (!empty($envData)) {
                        update_env_file($envData);
                    }
                }
